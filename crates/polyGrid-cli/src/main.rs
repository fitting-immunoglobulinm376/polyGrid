...
use exchange::{Exchange, PolymarketExchange, SimExchange};
use grid_engine::{
...
#[derive(Parser)]
#[command(name = "polyGrid-cli", about = "polyGrid command line")]
struct Cli {
    #[command(subcommand)]
    cmd: Commands,
}
...
enum Commands {
    ...
    /// Fetch mid price from Polymarket Perps
    Mid {
        #[arg(long, default_value = "BTC")]
        symbol: String,
    },
...
async fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Commands::Mid { symbol } => {
            ...
            pm.connect().await?;
            let mid = pm.get_mid(&symbol).await?;
