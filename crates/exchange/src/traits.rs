...
pub struct Balance {
    /// Coin / symbol code, e.g. pUSD, BTC. For account mode rows use the mode id.
    pub asset: String,
    pub total: Decimal,
...
pub struct MarketInfo {
    /// Key used for mid lookup / orders (e.g. BTC, BTC-USD)
    pub symbol: String,
    ...
    /// 24h notional volume in quote (pUSD), when available.
    #[serde(default)]
