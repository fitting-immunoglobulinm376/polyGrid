...
impl SimExchange {
    ...
    async fn advance_mid(&self) -> Decimal {
        ...
        let pull = -0.14 * dist - 0.18 * dist.powi(3);

        // Momentum with friction + gaussian-ish noise (kept milder than pull).
        let shock = rng.gen_range(-1.0..1.0) + 0.45 * rng.gen_range(-1.0..1.0);
        // Rare small impulse — large jumps skew inventory and delay round-trips.
        let jump = if rng.gen_bool(0.04) {
            rng.gen_range(-1.0..1.0) * 0.018 * range
        } else {
            0.0
        };

        st.velocity = st.velocity * 0.78 + pull * amp_f * 0.055 + shock * amp_f * 0.018;
        // Cap velocity so we don't teleport across the whole band in one tick.
        let vmax = 0.045 * range;
        ...
        let hi = hi_f - pad;
        if next < lo {
            next = lo + (lo - next) * 0.45;
            st.velocity = st.velocity.abs() * 0.65;
        } else if next > hi {
            next = hi - (next - hi) * 0.45;
            st.velocity = -st.velocity.abs() * 0.65;
        }

        // Micro noise for candle/line texture.
        next += rng.gen_range(-0.0005..0.0005) * range;
    ...
    async fn maybe_fill(&self) {
        ...
        let mut quote = self.quote_balance.lock().await;
...
impl Exchange for SimExchange {
    ...
    async fn get_balances(&self) -> ExchangeResult<Vec<Balance>> {
        let quote = *self.quote_balance.lock().await;
        let base = *self.base_balance.lock().await;
        Ok(vec![
            Balance {
                asset: "pUSD".into(),
                total: quote,
    ...
    async fn place_order(&mut self, intent: OrderIntent) -> ExchangeResult<LiveOrder> {
        // Mirror live exchanges: reduce-only must shrink position and not flip through flat.
...
mod tests {
    ...
    #[tokio::test]
    async fn fill_updates_quote_balance() {
        let mut sim = SimExchange::with_band(
            "BTC",
            dec!(100),
            dec!(1000),
        ...
        sim.force_mid(dec!(98)).await;
        let _ = sim.get_mid("BTC").await;
        let bals = sim.get_balances().await.unwrap();
        let quote = bals
            .iter()
            .find(|b| b.asset == "pUSD")
            .expect("quote")
            .total;
        assert_eq!(quote, dec!(901)); // 1000 - 99*1
        assert_eq!(sim.position_size().await, dec!(1));
    }

    #[tokio::test]
    async fn mean_reverting_band_stays_inside_and_set_band_recenters() {
        let sim = SimExchange::with_band(
            "BTC",
            dec!(100),
            dec!(10000),
            dec!(0),
        ...
        for _ in 0..200 {
            let mid = sim.get_mid("BTC").await.unwrap();
            assert!(mid >= dec!(90) && mid <= dec!(110), "mid={mid}");
        }
        sim.set_band(dec!(200), dec!(220)).await;
        let mid = sim.peek_mid().await;
        assert!(mid >= dec!(200) && mid <= dec!(220), "mid after set_band={mid}");
        for _ in 0..50 {
            let mid = sim.get_mid("BTC").await.unwrap();
            assert!(mid >= dec!(200) && mid <= dec!(220), "mid={mid}");
        }
    }

    #[tokio::test]
    async fn in_band_grid_accumulates_realized_profit() {
        use grid_engine::{
            BreakoutAction, GridConfig, GridEngine, GridMode, GridSpacing, MarketKind, RunMode,
        };
        ...
        let cfg = GridConfig {
            ...
            breakout_action: BreakoutAction::AlertOnly,
            max_drawdown_pct: Decimal::ZERO,
            max_daily_loss: dec!(9999),
            max_order_failures: 50,
        ...
        sim.connect().await.unwrap();
        let mut engine = GridEngine::new(cfg.clone(), RunMode::Simulation, cfg.total_budget).unwrap();
        let intents = engine.bootstrap_intents(mid0).unwrap();
        for order in sim.place_orders(intents).await.unwrap() {
            engine.register_live_order(order);
        }

        let mut best_realized = Decimal::ZERO;
        for _ in 0..400 {
            let mid = sim.get_mid("BTC").await.unwrap();
            let _ = engine.on_mid_price(mid);
            for fill in sim.drain_fills().await.unwrap() {
                if let Ok((_pnl, replenish)) = engine.on_fill(fill) {
                    if let Some(intent) = replenish {
                        if let Ok(order) = sim.place_order(intent).await {
                            engine.register_live_order(order);
                        }
                    }
                }
            ...
            if snap.realized_pnl > best_realized {
                best_realized = snap.realized_pnl;
            }
        }
        let snap = engine.snapshot();
        let total = snap.realized_pnl + snap.unrealized_pnl;
        assert!(
            best_realized > Decimal::ZERO || total > Decimal::ZERO,
            "expected grid profit; realized={} unrealized={} fills={}",
            snap.realized_pnl,
            snap.unrealized_pnl,
            snap.fill_count
        );
        assert!(
            snap.fill_count > 0,
            "expected fills from mean-reverting path"
        );
        assert!(
            !matches!(snap.status, grid_engine::BotStatus::Halted),
            "should not halt: {:?}",
            snap.status_note
        );
    }
