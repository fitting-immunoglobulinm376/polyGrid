...
#[serde(rename_all = "snake_case")]
pub enum MarketKind {
    /// Perpetual (default). Can open long/short without base inventory.
    Perp,
...
impl GridConfig {
    pub fn validate(&self) -> GridResult<()> {
        ...
        // Polymarket Perps min notional is typically $1.
        let per_level = self.total_budget / Decimal::from(self.grid_count);
        if per_level < Decimal::from(1) {
            return Err(GridError::InvalidConfig(format!(
                "每格名义约 {per_level} pUSD，低于交易所最低约 $1。请提高总投入或减少网格数量。"
            )));
        }
...
pub struct OrderIntent {
    ...
    #[serde(default)]
    pub tif: TimeInForce,
    /// Stable exchange client order id (32-char hex coid), when set.
    #[serde(default)]
...
pub struct FillEvent {
    ...
    /// Exchange trade id when known (Polymarket trade_id).
    #[serde(default)]
