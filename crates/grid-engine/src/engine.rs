...
impl GridEngine {
    ...
    /// Hard-halt ceiling for inventory notional.
    ///
    /// `max_position_notional` is one grid side at level prices. Holding that
    /// inventory across the band marks it higher/lower by roughly the band
    /// width, so a tight 2% buffer false-halts normal grid oscillation.
    fn position_halt_notional_limit(&self) -> Decimal {
        if self.max_position_notional <= Decimal::ZERO {
            return Decimal::MAX;
        }
        let mid = (self.active_lower + self.active_upper) / Decimal::from(2);
        let band_pct = if mid > Decimal::ZERO {
            ((self.active_upper - self.active_lower) / mid).max(Decimal::ZERO)
        } else {
            Decimal::ZERO
        };
        // Full-band MTM drift + 10% slack for partial fills / rounding; floor at 1.25×.
        let factor =
            (Decimal::ONE + band_pct + Decimal::new(10, 2)).max(Decimal::new(125, 2));
        self.max_position_notional * factor
    }
    ...
    ///
    /// Exchanges reject `reduce_only` that would flip through flat
    ...
    pub fn on_mid_price(&mut self, price: Decimal) -> Vec<EngineEvent> {
        ...
        if wants_recenter || matches!(action, BreakoutAction::Recenter) {
            ...
            let confirmed = self.outside_confirm_bars >= self.config.dynamic.confirm_bars.max(1)
                || self.soft_breakout_count >= self.config.dynamic.confirm_bars.max(1) * 60;
            // soft_breakout_count is tick-based (~1s); prefer candle confirms from runner.
            // Fall back: many ticks ≈ confirm if runner hasn't fed candles yet.
            let tick_fallback = self.soft_breakout_count >= 60; // ~60s at 1s/tick
    ...
    pub fn on_fill(&mut self, fill: FillEvent) -> GridResult<(Decimal, Option<OrderIntent>)> {
        ...
        self.risk
            .on_strategy_equity(unrealized + self.funding_pnl, &self.risk_cfg);
        // Use mid when available so a single print at the band edge does not
        // inflate notional vs the level-priced side budget.
        let mark = self.mid_price.unwrap_or(fill.price);
        let position_notional = self.position_base.abs() * mark;
        let position_limit = self.position_halt_notional_limit();
        let fill_was_expanding = match fill.side {
            Side::Buy => position_before >= Decimal::ZERO,
            Side::Sell => position_before <= Decimal::ZERO,
        };
        if matched_order
            && fill_was_expanding
            && self.max_position_notional > Decimal::ZERO
            && position_notional > position_limit
        {
            self.risk.force_halt(format!(
                "position notional {position_notional} exceeds grid-side limit {position_limit} (side budget {})",
                self.max_position_notional
            ));
        }
        self.push_event(format!(
...
mod tests {
    ...
    #[test]
    fn position_halt_allows_band_mtm_overshoot() {
        // Same shape as the user's halt: side budget ~150, mark notional ~162.
        let mut cfg = sample_cfg();
        cfg.lower_price = dec!(1862);
        cfg.upper_price = dec!(1977);
        cfg.grid_count = 30;
        cfg.total_budget = dec!(300);
        let mut engine = GridEngine::new(cfg, RunMode::Simulation, dec!(1000)).unwrap();
        let intents = engine.bootstrap_intents(dec!(1920)).unwrap();
        let side = engine.max_position_notional();
        assert!(side > Decimal::ZERO, "side={side}");
        let limit = engine.position_halt_notional_limit();
        // Old 1.02× limit false-halted ~8% MTM overshoot; new floor is ≥1.25×.
        assert!(limit >= side * dec!(1.25), "limit={limit} side={side}");
        assert!(dec!(161.82228) < limit);

        let buy = intents
            .iter()
            .find(|i| i.side == Side::Buy)
            .expect("bootstrap should include buys");
        let order = LiveOrder::from_intent(buy, Some("1".into()));
        engine.register_live_order(order.clone());
        let fill_size = (dec!(161.82228) / dec!(1920)).round_dp(8);
        let fill = FillEvent {
            client_id: order.client_id.clone(),
            symbol: order.symbol.clone(),
            side: Side::Buy,
        ...
        let result = engine.on_fill(fill);
