...
impl Storage {
    pub fn open_default() -> Result<Self> {
        ...
        let db_path = root.join("polyGrid.db");
        ...
        if !db_path.exists() {
            ...
            let beside = program_dir.join("polyGrid.db");
            if beside.exists() {
                fs::copy(&beside, &db_path).with_context(|| {
                    format!("migrate db from {} to {}", beside.display(), db_path.display())
                })?;
                for suffix in ["-wal", "-shm"] {
                    let src = program_dir.join(format!("polyGrid.db{suffix}"));
                    if src.exists() {
                        let _ = fs::copy(&src, root.join(format!("polyGrid.db{suffix}")));
                    }
                }
                migrated_from = Some(beside);
            } else if let Some(dirs) = ProjectDirs::from("xyz", "polyGrid", "polyGrid") {
                // 2) Legacy XDG app-data location.
                let legacy_dir = dirs.data_dir();
                let legacy_db = legacy_dir.join("polyGrid.db");
                if legacy_db.exists() {
                    ...
                    for suffix in ["-wal", "-shm"] {
                        let src = legacy_dir.join(format!("polyGrid.db{suffix}"));
                        if src.exists() {
                            let _ = fs::copy(&src, root.join(format!("polyGrid.db{suffix}")));
                        }
                    }
