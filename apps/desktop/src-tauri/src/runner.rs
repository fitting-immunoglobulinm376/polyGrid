...
/// Main loop period (1s). Position/ATR/funding/order sync stay throttled below.
const TICK_INTERVAL_MS: u64 = 1000;
/// Sync exchange position every N ticks (~9s at 1s/tick).
const POSITION_SYNC_EVERY_TICKS: u32 = 9;
/// Sync open orders from exchange every N ticks (~15s) to drop phantoms.
const ORDER_SYNC_EVERY_TICKS: u32 = 15;
/// Refresh ATR / funding about once per minute.
const SLOW_POLL_EVERY_TICKS: u32 = 60;
/// Persist equity/checkpoint about every 30s.
const CHECKPOINT_EVERY_TICKS: u32 = 30;

use exchange::{
    fetch_candles, CandleInterval, Exchange, PolymarketExchange,
};
use grid_engine::{
    compute_atr, derive_bounds, is_outside_bounds, suggest_half_width_pct, AtrMetrics, BotSnapshot,
    BotStatus, BreakoutAction, DynamicGridConfig, EngineEvent, GridConfig, GridEngine, GridMode,
    MarketKind, OhlcBar, RunMode,
};
...
pub async fn protect_symbol(
    ...
    let pm = st.pm.as_mut().ok_or("Polymarket exchange unavailable")?;
    let report = pm
    ...
    if close_position {
        pm.close_position(symbol).await.map_err(|e| e.to_string())?;
        let (remaining, _, _, _) = pm
...
pub async fn execute_recenter(
    ...
    let cancel_ok = if st.mode == RunMode::Simulation {
        ...
        let pm = st.pm.as_mut().ok_or("no pm")?;
        let report = pm
            .cancel_all_confirmed(&symbol, 8)
            .await
            .map_err(|e| e.to_string())?;
        ...
        if st.mode != RunMode::Simulation {
            if let Some(pm) = st.pm.as_mut() {
                let lev = st
                    ...
                    .unwrap_or(5);
                if let Err(e) = pm.preflight_grid_notional(&intents, lev).await {
    ...
    // Keep the in-band oscillator aligned with the new grid (sim only).
    if st.mode == RunMode::Simulation {
        if let Some(sim) = st.sim.as_ref() {
            sim.set_band(plan.lower, plan.upper).await;
        }
    }
...
async fn sync_open_orders_from_exchange(
    ...
    if let Some(pm) = st.pm.as_mut() {
        pm.adopt_open_orders(symbol, &merged);
    }
    Ok(replenish)
}
...
async fn refresh_atr(mode: RunMode, engine: &mut GridEngine) -> Result<(), String> {
    // Simulation mid is an in-band oscillator — never confirm breakouts from live klines.
    if mode == RunMode::Simulation {
        let (lo, hi) = engine.active_bounds();
        let mid = engine
            .mid_price
            .unwrap_or_else(|| (lo + hi) / Decimal::from(2));
        engine.note_outside_confirm_bar(is_outside_bounds(mid, lo, hi));
        // Derive atr_pct so half_width ≈ atr_pct * atr_mult keeps the current band.
        let half_pct = if mid > Decimal::ZERO {
            ((hi - lo) / Decimal::from(2) / mid) * Decimal::from(100)
        } else {
            Decimal::from(5)
        };
        let atr_mult = engine.config.dynamic.atr_mult.max(Decimal::new(1, 1));
        let atr_pct = (half_pct / atr_mult).round_dp(6);
        let atr = (mid * atr_pct / Decimal::from(100)).round_dp(8);
        engine.set_atr(&AtrMetrics {
            atr,
            atr_pct,
            period: engine.config.dynamic.atr_period,
            bar_count: 0,
        });
        return Ok(());
    }
...
async fn sync_fills_and_position(
    ...
    if st.mode != RunMode::Simulation {
        if let Some(pm) = st.pm.as_mut() {
            if let Ok((size, entry, upnl, liq)) = pm.get_perp_position(symbol).await {
                if let Some(engine) = st.engine.as_mut() {
...
pub async fn tick_once(
    ...
    *position_poll_tick = position_poll_tick.wrapping_add(1);
    ...
    {
        if let Some(pm) = st.pm.as_mut() {
            match pm.get_perp_position(&symbol).await {
    ...
    {
        if let Some(pm) = st.pm.as_ref() {
            match pm.get_session_funding_pnl(&symbol).await {
                Ok(funding_pnl) => {
                    if let Some(engine) = st.engine.as_mut() {
...
pub fn detach_on_exit(st: &mut AppState) {
    ...
    // Simulation books live only in memory — detaching leaves an "active" session that
    // the next launch tries to resume against an empty SimExchange and can wedge startup.
    if st.mode == RunMode::Simulation || engine.mode == RunMode::Simulation {
        let payload = engine.checkpoint_payload();
        let _ = st.storage.save_checkpoint(&sid, "exit_sim", &payload);
        let _ = st.storage.deactivate_session(&sid, Some("stopped"));
        let _ = st.storage.record_event(
            "exit_sim",
            "simulation window closed; session not resumable",
        );
        info!("simulation exit: session {sid} deactivated (not resumable)");
        return;
    }
...
pub async fn try_resume_active_session(app: &AppHandle, st: &mut AppState) -> Result<bool, String> {
    ...
    // SimExchange state is process-local. Resuming against a fresh empty book treats every
    // checkpoint order as a phantom fill and can lock up startup while holding AppState.
    if checkpoint_mode == Some(RunMode::Simulation) {
        let _ = st
            .storage
            .deactivate_session(&session.session_id, Some("stopped"));
        info!(
            "skip resume: simulation session {} is not durable across restarts",
            session.session_id
        );
        return Ok(false);
    }
    if st.mode == RunMode::Simulation {
        info!(
            "skip resume: app is in simulation mode (session {} left intact)",
            session.session_id
        );
        return Ok(false);
    }
    ...
    if st.pm.is_none() {
        let mut pm = PolymarketExchange::new(mode);
    ...
    st.pm
        ...
        .await
        .map_err(|e| e.to_string())?;
    if let Err(e) = st.pm.as_mut().unwrap().prime_seen_fills().await {
    ...
    if let Err(e) = recover_session(app, st).await {
        // Avoid leaving a stuck "running" engine with no tick loop.
        st.running_task = false;
        if let Some(engine) = st.engine.as_mut() {
            let sid = engine.session_id().to_string();
            persist_checkpoint(&st.storage, engine, "resume_failed");
            let _ = st.storage.deactivate_session(&sid, Some("stopped"));
        }
        st.engine = None;
        return Err(e);
